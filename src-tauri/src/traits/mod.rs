...
pub trait Storable
where
    Self: Sized + serde::de::DeserializeOwned + serde::ser::Serialize + Clone + Entity,
{
    ...
    fn get(handle: &tauri::AppHandle, key: &str) -> Result<Self, tauri_plugin_store::Error> {
        let json_value = handle.store(STORE_URI)?.get(key).ok_or_else(|| {
            tauri_plugin_store::Error::Io(std::io::Error::from(std::io::ErrorKind::NotFound))
        })?;
