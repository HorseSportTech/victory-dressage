	...
	fn default() -> Self {
		Self {
			...
			status: {
				...
				map.insert(StarterResult::InProgress(0).to_string(), PositionedWarning::default());
				map
			},
...
impl PositionedWarning {
	...
	pub fn toggle(&mut self, pos: Position) -> bool {
		match pos {
			Position::K => {self.k = !self.k; self.k},
			Position::E => {self.e = !self.e; self.e},
			Position::H => {self.h = !self.h; self.h},
			Position::C => {self.c = !self.c; self.c},
			Position::M => {self.m = !self.m; self.m},
			Position::B => {self.b = !self.b; self.b},
			Position::F => {self.f = !self.f; self.f},
		}
	}
}